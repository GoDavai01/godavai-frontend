// src/pages/SearchResults.js — GoDavaii 2030 Ultra Modern (FIXED)
// ✅ ALL LOGIC 100% UNCHANGED
// ✅ FIXED: Autocomplete dropdown was getting CLIPPED (parent had overflow:hidden)
// ✅ FIXED: Dropdown layering + touch selection reliability
// ✅ NEW: Small layout polish (still visual-only)
...
function MedCard({ med, onAdd, onOpen }) {
  ...
  const origPrice =
    med.mrp && med.price && Number(med.price) < Number(med.mrp) ? med.mrp : null;
...
export default function SearchResults() {
    ...
    async function run() {
      ...
      try {
        ...
        const chips = Array.from(
          new Set(meds.map((m) => m.brand || m.name).filter(Boolean).map((s) => String(s)))
        ).slice(0, 12);
  ...
  // ── Live autocomplete while typing — IDENTICAL ─────────────
  useEffect(() => {
    if (!localQuery.trim()) {
      setLiveOptions([]);
      return;
  ...
  // ── Click outside closes dropdown — IDENTICAL ──────────────
  useEffect(() => {
    const h = (e) => {
  ...
  const images = useMemo(() => {
    ...
    const arr = (
      Array.isArray(selectedMed.images) && selectedMed.images.length ? selectedMed.images : [selectedMed.img]
    ).filter(Boolean);
    return arr;
  }, [selectedMed]);
  ...
  const liveOptionLabels = liveOptions
  ...
  return (
    ...
    >
      {/* ═══ HEADER ═══ */}
      {/* ✅ FIX: overflow must be visible so dropdown isn't clipped */}
      <div
        style={{
          background: `linear-gradient(160deg,${DEEP} 0%,#083D28 70%,#041F14 100%)`,
          padding: "18px 16px 20px",
          position: "relative",
          overflow: "visible", // 🔥 FIXED (was hidden -> clipped dropdown)
          zIndex: 50, // ensure header sits above content
        }}
      >
        <div
          style={{
            ...
            {!loading && totalResults > 0 && (
              <motion.span
                ...
                style={{
                  ...
                  borderRadius: 100,
                  flexShrink: 0,
                  boxShadow: "0 0 12px rgba(0,217,126,0.4)",
                  display: "flex",
                  alignItems: "center",
                  gap: 4,
                }}
        ...
        <div ref={boxRef} style={{ position: "relative", zIndex: 60 }}>
          <motion.div
            animate={{
          ...
          >
            ...
            <div
              style={{
                ...
                background: searchFocused
                  ? `linear-gradient(135deg,${DEEP}18,${ACCENT}15)`
                  : "#F1F5F9",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
            ...
            <motion.button
              whileTap={{ scale: 0.88 }}
              onMouseDown={(e) => e.preventDefault()} // ✅ avoid blur flicker
              ...
              style={{
                ...
                background: micActive
                  ...
                  : `linear-gradient(135deg,${ACCENT}20,#00E5FF18)`,
            ...
            >
              ...
              {micActive ? (
                <MicOff style={{ width: 16, height: 16, color: "#fff", position: "relative", zIndex: 1 }} />
              ) : (
                <Mic style={{ width: 16, height: 16, color: DEEP }} />
              )}
            </motion.button>
          ...
          <AnimatePresence>
            {dropdownOpen && (
              ...
              >
                ...
                >
                  {localQuery ? (
                    <Sparkles style={{ width: 11, height: 11, color: ACCENT }} />
                  ) : (
                    <TrendingUp style={{ width: 11, height: 11, color: "#94A3B8" }} />
                  )}
                  <span
                    style={{
                      fontSize: 10,
                  ...
                  >
                    {localQuery ? "Suggestions" : "Trending Searches"}
                  </span>
                </div>
                ...
                {(localQuery ? liveOptionLabels : TRENDING_SEARCHES).map((label, i) => (
                  <motion.button
                    ...
                    // ✅ FIX: onMouseDown + onTouchStart so it works on mobile and doesn't lose click on blur
                    onMouseDown={(e) => {
                      e.preventDefault();
                    ...
                    style={{
                      ...
                      textAlign: "left",
                      borderBottom:
                        i < (localQuery ? liveOptionLabels : TRENDING_SEARCHES).length - 1
                  ...
                  >
                    <div
                      style={{
                        ...
                        height: 34,
                        borderRadius: 11,
                        background: localQuery ? "#E8F5EF" : "#F1F5F9",
                        display: "flex",
                        alignItems: "center",
                        justifyContent: "center",
                        flexShrink: 0,
                    ...
                    >
                      {localQuery ? (
                        <Search style={{ width: 14, height: 14, color: DEEP }} />
                      ) : (
                        <TrendingUp style={{ width: 14, height: 14, color: "#94A3B8" }} />
                      )}
                    </div>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {localQuery && label.toLowerCase().includes(localQuery.toLowerCase()) ? (
                        ...
                        >
                          <span style={{ color: "#94A3B8" }}>
                            {label.substring(0, label.toLowerCase().indexOf(localQuery.toLowerCase()))}
                          </span>
                          <span style={{ color: DEEP, fontWeight: 800 }}>
                            {label.substring(
                              label.toLowerCase().indexOf(localQuery.toLowerCase()),
                              label.toLowerCase().indexOf(localQuery.toLowerCase()) + localQuery.length
                            )}
                          </span>
                          <span style={{ color: "#94A3B8" }}>
                            {label.substring(label.toLowerCase().indexOf(localQuery.toLowerCase()) + localQuery.length)}
                          </span>
                        </span>
                      ) : (
                        <span
                      ...
                      {!localQuery && (
                        <span style={{ fontSize: 10, color: "#CBD5E1", fontWeight: 500 }}>Popular search</span>
                      )}
                    </div>
                ...
                {localQuery && liveOptionLabels.length === 0 && !liveLoading && (
                  <div style={{ padding: "16px", display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 700, color: "#0B1F16" }}>
                  ...
                  {[0, 1, 2].map((i) => (
                    <div
                      ...
                      style={{
                        ...
                        borderRadius: 2,
                        background: "#fff",
                        animation: `micWave 0.8s ease-in-out ${i * 0.15}s infinite`,
                      }}
                    />
                  ))}
                </div>
                <span style={{ fontSize: 13, fontWeight: 700, color: "#fff", fontFamily: "'Sora',sans-serif" }}>
                  Listening... Speak now
                </span>
              </motion.div>
            )}
      ...
      <div style={{ padding: "16px 14px 0" }}>
        {/* Post-search suggestion chips — IDENTICAL logic */}
        <AnimatePresence>
                ...
                {autoSuggestions.map((s) => (
                  <motion.button
                    ...
                    style={{
                      ...
                      fontWeight: 700,
                      fontFamily: "'Plus Jakarta Sans',sans-serif",
                      boxShadow: s === query ? "0 4px 14px rgba(12,90,62,0.25)" : "0 2px 8px rgba(0,0,0,0.05)",
                    }}
                  >
                    {s}
                  </motion.button>
        ...
        {!loading && pharmacySections.length === 0 && (
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} style={{ textAlign: "center", padding: "60px 20px" }}>
            ...
            <div style={{ fontFamily: "'Sora',sans-serif", fontSize: 18, fontWeight: 900, color: "#0B1F16", marginBottom: 8 }}>
              No medicines found
            </div>
            <div style={{ fontSize: 13, color: "#94A3B8", lineHeight: 1.7 }}>
              {query ? (
                <>
                  <span>No nearby pharmacies have </span>
                  <strong>"{query}"</strong>
                  <br />
        ...
        {/* Results by pharmacy — IDENTICAL logic */}
        {!loading &&
              ...
              <div style={{ padding: "14px 14px 14px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    >
                      <div style={{ fontFamily: "'Sora',sans-serif", fontSize: 15, fontWeight: 800, color: "#0B1F16", letterSpacing: "-0.2px" }}>
                        {pharmacy.name}
                      </div>
              ...
              <div style={{ padding: "14px 20px 0" }}>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8 }}>
                  {Array.isArray(selectedMed.category) &&
                    selectedMed.category.map((c, i) => (
                      <span key={i} style={{ fontSize: 11, fontWeight: 600, color: DEEP, background: "#E8F5EF", padding: "2px 9px", borderRadius: 100 }}>
                        {c}
                      </span>
                    ))}
                </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                  <span style={{ fontFamily: "'Sora',sans-serif", fontSize: 26, fontWeight: 900, color: DEEP }}>
