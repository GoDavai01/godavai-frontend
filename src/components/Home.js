...
export default function Home() {
  ...
  // Fetch doctor consults for patient
  useEffect(() => {
    const token = localStorage.getItem("token");
    if (!token) return;
    axios.get(`${API}/api/consults/my`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  return (
    <div style={{ minHeight: "100vh", width: "100%", maxWidth: 480, margin: "0 auto", background: "linear-gradient(180deg,#F2F7F4 0%,#E8F5EF 30%,#F0F9FF 60%,#F5F3FF 80%,#F2F7F4 100%)", paddingBottom: 120, position: "relative", overflowX: "hidden", fontFamily: "'Plus Jakarta Sans',sans-serif" }}>
      ...
      <div style={{ padding: "18px 18px 0" }}>
        ...
        {myConsults.filter(c => ["confirmed","accepted","live_now","completed"].includes(c.status)).length > 0 && (
          <div style={{ marginBottom: 18 }}>
            <Section title="Doctor Consultations" badge="LIVE" onSeeAll={() => navigate("/doctors")} />
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {myConsults.filter(c => ["confirmed","accepted","live_now","completed"].includes(c.status)).slice(0, 3).map((c) => {
                ...
                return (
                  <Glass key={c.id} style={{ padding: 14, overflow: "hidden", position: "relative" }}>
                    {isLive && <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: "linear-gradient(90deg,#10B981,#34D399,#10B981)", backgroundSize: "200% 100%", animation: "hmLiveBar 1.5s linear infinite" }} />}
                    <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
                      <div style={{ width: 44, height: 44, borderRadius: 16, background: "linear-gradient(135deg,#E8F5EF,#D1FAE5)", display: "grid", placeItems: "center", fontSize: 16, fontWeight: 900, color: DEEP, flexShrink: 0 }}>🩺</div>
                      <div style={{ minWidth: 0, flex: 1 }}>
                        <div style={{ fontFamily: "'Sora',sans-serif", fontSize: 13.5, fontWeight: 1000, color: "#0B1F16" }}>{c.doctorName}</div>
                        <div style={{ fontSize: 10.5, color: "#64748B", fontWeight: 700 }}>{c.specialty} · {c.dateLabel} · {c.slot}</div>
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 4, flexShrink: 0 }}>
                        {isLive && <span style={{ width: 7, height: 7, borderRadius: "50%", background: "#10B981", animation: "glowPulse 1.5s ease-in-out infinite" }} />}
                        <span style={{ fontSize: 10, fontWeight: 900, color: isLive ? "#065F46" : c.status === "completed" ? "#065F46" : "#92400E", background: isLive ? "#D1FAE5" : c.status === "completed" ? "#ECFDF5" : "#FEF3C7", padding: "4px 10px", borderRadius: 999, border: `1px solid ${isLive ? "#A7F3D0" : c.status === "completed" ? "#A7F3D0" : "#FDE68A"}` }}>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      {canJoin && (
                        <motion.button whileTap={{ scale: 0.95 }} onClick={() => { const roomId = c.consultRoomId || `consult_${c.id?.slice(-8)}`; window.open(`https://meet.jit.si/${roomId}`, "_blank"); }}
                          style={{ flex: 1, height: 38, border: "none", borderRadius: 12, background: isLive ? "linear-gradient(135deg,#059669,#10B981)" : `linear-gradient(135deg,${DEEP},${MID})`, color: "#fff", fontFamily: "'Sora',sans-serif", fontWeight: 900, fontSize: 11.5, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, boxShadow: "0 6px 18px rgba(12,90,62,0.2)" }}>
                          {c.mode === "video" ? "📹" : "📞"} {isLive ? "Join Now" : "Join Consult"}
                        </motion.button>
                      )}
                      {hasPrescription && (
                        <a href={c.prescription.fileUrl} target="_blank" rel="noreferrer" style={{ flex: canJoin ? "none" : 1, height: 38, borderRadius: 12, background: "#F0FDF4", border: "1px solid #BBF7D0", color: "#15803D", fontWeight: 900, fontSize: 11.5, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 5, padding: "0 14px", textDecoration: "none", fontFamily: "'Sora',sans-serif" }}>
                          📋 Prescription
                        </a>
                      )}
                      {!canJoin && !hasPrescription && c.status === "completed" && (
                        <div style={{ flex: 1, height: 38, borderRadius: 12, background: "#FFFBEB", border: "1px solid #FDE68A", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, color: "#92400E" }}>⏳ Prescription pending</div>
                      )}
                    </div>
                  </Glass>
                );
